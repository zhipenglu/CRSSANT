...
def getreads(alignfile): #verified
    ...
    if(alignfile.endswith("sam")):
        print(alignfile," is SAM")
        try:
            inputbam = pysam.AlignmentFile(alignfile, "r", require_index=False)
        except OSError as e:
            sys.exit()
    if(alignfile.endswith(".bam")):
        print(alignfile, " is BAM")
        try:
            inputbam = pysam.AlignmentFile(alignfile, "rb", require_index=False)
        except OSError as e:
            sys.exit()
    if(inputbam is None):
...
def buildGeneAlignDict(inputfile, genesdict): #verified
    ...
    for align in inputbam:
        ...
        if(QNAME != current_qname):
            #TODO process all aligns for QNAME into the genesdict
            align1 = current_aligns[0]
            align2 = current_aligns[1]

            #lines = readsdict[QNAME] #all alignments for this read
            if(align0.has_tag('SA')):
